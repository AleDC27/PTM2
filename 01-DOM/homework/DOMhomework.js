...
let toDoText=document.createElement("spam");
...
function buildToDos(toDos) {
  // Tu código acá:
  let arr=toDos.map(function(cur,i){
    return buildToDo(cur,i)
  });
  return arr;
}
...
function displayToDos() {
  ...
  let buil=buildToDos(toDoItems);
  for (let i = 0; i < buil.length; i++){
    toDoContainer.appendChild(buil[i]);
  };
}
